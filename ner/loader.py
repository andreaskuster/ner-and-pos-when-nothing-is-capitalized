...
def load_twitter_sentences(twitter_path):
    """
    Load sentences. A line must contain at least a word and its tag.
    Sentences are separated by empty lines.
    """
    sentences = []
    with open(twitter_path, mode='r') as f:
        for line in f.readlines():
            d = json.loads(line)
            l = [d['tokens'],d['entities']]
            sentences.append(list(map(list, zip(*l))))
    return sentences
...
def prepare_dataset_twitter(sentences, word_to_id, char_to_id, tag_to_id, lower=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
...
def prepare_dataset_mixed(sentences, word_to_id, char_to_id, tag_to_id, lower=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
        - tag indexes
    """
    def g(x): return x.lower() 
    data = []
    rand_samples = random.sample(range(0, len(sentences)), int(0.5*len(sentences)))
    for i in range(len(sentences)):
        s = sentences[i]
        str_words = [w[0] for w in s]
        if i in rand_samples:
            words = [word_to_id[g(w) if g(w) in word_to_id else '<UNK>']
                     for w in str_words]
        else:
...
def prepare_dataset_cased_uncased(sentences, word_to_id, char_to_id, tag_to_id, lower=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
...
def prepare_dataset_truecase(sentences, word_to_id, char_to_id, tag_to_id, lower=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
    ...
    truecase_input =[]
    for s in sentences:
        str_words = [g(w[0]) for w in s]
        # words = [word_to_id[g(w) if g(w) in word_to_id else '<UNK>']
        #          for w in str_words]
        truecase_input.append(str_words)
    truecase_output =  predict_truecasing(truecase_input)
    for i in range(len(sentences)):
        s = sentences[i]
        str_words = truecase_output[i]
        # str_words = [w[0] for w in s]
        words = [word_to_id[w if w in word_to_id else '<UNK>']
                 for w in str_words]
        # words = predict_truecasing(words)
        # Skip characters that are not in the training set
        chars = [[char_to_id[c] for c in w if c in char_to_id]
                 for w in str_words]
        caps = [cap_feature(w) for w in str_words]
...
def prepare_dataset_truecase_twitter(sentences, word_to_id, char_to_id, tag_to_id, lower=True):
    """
    Prepare the dataset. Return a list of lists of dictionaries containing:
        - word indexes
        - word char indexes
