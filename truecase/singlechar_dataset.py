...
class TrueCaseDatasetImproved(Dataset):
    def __init__(self, data: Union[str, List[List[str]]], token_dict: Dict[str, int] = None, train: bool = False, OOV_rate: float = 0.0):
        super().__init__()
        ...
        if token_dict is None:
            total_token_count = np.sum(list(token_counts.values()))
            sorted_values = sorted([(int(v), k) for k, v in token_counts.items()])
            curr_OOV_sum = 0
            idx = 0
            while OOV_rate > curr_OOV_sum / total_token_count:
                curr_OOV_sum += int(sorted_values[idx][0])
                idx += 1
            unique_tokens = np.array([k for _, k in sorted_values[idx:]])
